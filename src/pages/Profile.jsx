...
const Profile = () => {
  ...
  const handleSaveProfilePic = async () => {
    ...
    try {
      ...
      formData.append("profilePic", profilePic); // Send the actual file

      const response = await axios.post(
        `${import.meta.env.VITE_REACT_APP_BASE_URL}/updateUser/uploadProfilePic`,
        formData,
        { withCredentials: true }
      );
          ...
          <div className="w-full max-w-md">
            <form onSubmit={(e) => handleSubmit(e)} className="space-y-6">
              ...
              <div className="mt-6">
                <button
                  type="submit"
                  className="w-full flex justify-center px-3 py-2 bg-indigo-600 text-white rounded-full text-lg font-semibold shadow-md hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
                >
                  Save
                </button>
              </div>
